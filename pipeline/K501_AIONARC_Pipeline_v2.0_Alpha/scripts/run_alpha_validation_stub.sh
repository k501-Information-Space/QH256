...
export QH256_CORE_DIR="$ROOT/tests/stub_qh256"
"$ROOT/scripts/build.sh"
"$ROOT/scripts/build_verify.sh"
"$ROOT/tests/run_tests.sh"
rm -rf "$ROOT/build/fixture_input" "$ROOT/build/run_a" "$ROOT/build/run_b" "$ROOT/build/run_asan" "$ROOT/build/mutation_input_a" "$ROOT/build/mutation_input_b" "$ROOT/build/mutation_out_a" "$ROOT/build/mutation_out_b" "$ROOT/build/history_bad.bin"
...
import os,sys
r=os.fsencode(sys.argv[1]); n=b'invalid-'+bytes([255])+b'.bin'; fd=os.open(r+b'/'+n,os.O_WRONLY|os.O_CREAT|os.O_TRUNC,0o644); os.write(fd,b'raw-byte-path'); os.close(fd)
PY
...
import json,sys
m=json.load(open(sys.argv[1],encoding='utf-8')); assert m['count']==7; assert any('ff' in x['path_hex'] for x in m['records']); print('PASS: raw invalid-UTF8 path preserved')
PY
...
echo 'V2.0 ALPHA VALIDATION COMPLETE (QH256 interface stub; actual core validation is run via build.sh in the target QH256 tree)'
